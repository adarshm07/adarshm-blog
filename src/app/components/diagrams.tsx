...
export function PrototypeChainDiagram() {
  const boxes = [
    { label: 'dog', sub: "{ name: 'Rex' }", accent: true },
    { label: 'Dog.prototype', sub: '{ bark() }' },
    { label: 'Animal.prototype', sub: '{ eat() }' },
    { label: 'Object.prototype', sub: '{ toString(), … }' },
    { label: 'null', sub: 'end of the chain' },
  ]
  return (
    <div className="not-prose my-6 rounded-xl border border-neutral-100 dark:border-neutral-800 p-4">
      <svg
        ...
        role="img"
        aria-label="Prototype chain: a dog instance links via its internal prototype to Dog.prototype, then Animal.prototype, then Object.prototype, then null. A property lookup walks up this chain."
      >
        {boxes.map((box, i) => {
          const y = i * 95
          const isLast = box.label === 'null'
          return (
            <g key={box.label}>
              {i > 0 && (
                <>
              ...
              {isLast ? (
                <text
                  ...
                  y={y + 18}
                  textAnchor="middle"
                  className="fill-neutral-400 dark:fill-neutral-500 font-mono text-[12px]"
                >
      ...
      <p className="mt-3 text-xs text-center text-neutral-500 dark:text-neutral-400">
        Reading <code>dog.eat()</code> fails on the instance and on
        Dog.prototype, then succeeds on Animal.prototype — the engine walks up
